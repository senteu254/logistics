...
<div class="data-card">
    ...
    $dispStmt = $pdo->prepare("
        SELECT
            ...
            SUM(CASE WHEN dispatch_status='transit'  THEN 1 ELSE 0 END) AS transit,
            SUM(CASE WHEN dispatch_status='received' THEN 1 ELSE 0 END) AS received,
            SUM(CASE WHEN dispatch_status='rejected' THEN 1 ELSE 0 END) AS rejected,
            COUNT(*) AS total
        FROM bl_items WHERE bl_id = ?
    ");
    $dispStmt->execute([$bl['id']]);
    $ds = $dispStmt->fetch();
    ?>
    <div style="display:flex;flex-direction:column;gap:3px;min-width:110px;">
        <div style="height:5px;background:var(--border);border-radius:99px;overflow:hidden;display:flex;">
            <?php if ($ds['total'] > 0): ?>
            <div style="width:<?php echo round(($ds['received']/$ds['total'])*100); ?>%;background:var(--emerald);height:100%;"></div>
            <div style="width:<?php echo round(($ds['transit']/$ds['total'])*100); ?>%;background:var(--amber);height:100%;"></div>
            <div style="width:<?php echo round(($ds['rejected']/$ds['total'])*100); ?>%;background:var(--rose);height:100%;"></div>
            <?php endif; ?>
        </div>
        <div style="font-size:0.7rem;color:var(--txt-3);font-weight:500;">
            <?php echo $ds['received']; ?>/<?php echo $ds['total']; ?> received
            <?php if ($ds['transit'] > 0): ?>
            · <span style="color:var(--amber);"><?php echo $ds['transit']; ?> transit</span>
            <?php endif; ?>
        </div>
    </div>
</td>
