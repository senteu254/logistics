...
if ($type_f)   { $where[] = "b.bl_type = ?"; $params[] = $type_f; }
...
@media (min-width: 768px) {
  .dash-wrapper { padding: 2rem 1.5rem 3rem; }
}
...
.ship-table tbody tr:hover { background: #f9fafb; }
.ship-table tbody td {
  padding: .75rem 1rem;
...
.bl-num:hover { text-decoration: underline; color: #1d4ed8; }
...
.type-badge.tbl     { background: #eff6ff; color: #1d4ed8; }
.type-badge.non-tbl { background: #fef3c7; color: #92400e; }
...
.status-badge.completed { background: #eff6ff; color: #1d4ed8; }
...
.act-btn.view:hover  { color: var(--accent-blue); }
.act-btn.edit:hover  { color: #374151; }
.act-btn.del:hover   { color: var(--accent-red); border-color: #fca5a5; background: #fef2f2; }
...
.ship-stat + .ship-stat { border-left: 1px solid var(--border); }
.ship-stat-label {
  font-size: .625rem;
  text-transform: uppercase;
  letter-spacing: .04em;
...
.pager-btn:hover   { background: #f3f4f6; color: var(--accent-blue); }
.pager-btn.active  { background: var(--accent-blue); color: #fff; border-color: var(--accent-blue); font-weight: 600; }
...
.dash-alert.success { background: #f0fdf4; color: #15803d; border: 1px solid #bbf7d0; }
...
.btn-primary-dash:hover  { background: #1d4ed8; color: #fff; }
.btn-primary-dash:active { transform: scale(.97); }
        ...
        <table class="ship-table" aria-label="Shipments list">
          ...
          <tbody>
            <?php foreach ($bills as $bl):
              $sc = match($bl['status']) { 'active'=>'active','draft'=>'draft',default=>'completed' };
