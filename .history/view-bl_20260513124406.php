...
.summary-tile-ico { font-size:1.4rem;margin-bottom:0.6rem; }
.summary-tile-val { font-size:1.35rem;font-weight:800;color:var(--txt-1);letter-spacing:-0.3px;line-height:1;margin-bottom:4px; }
.summary-tile-lbl { font-size:0.72rem;font-weight:600;color:var(--txt-2);text-transform:uppercase;letter-spacing:0.05em; }
...
.container-row-card { background:var(--bg);border:1.5px solid var(--border);border-radius:var(--r-md);overflow:hidden;transition:var(--t); }
.container-row-card:hover { border-color:var(--indigo);box-shadow:0 3px 16px rgba(99,102,241,0.08); }
.container-row-num  { background:linear-gradient(135deg,var(--indigo),var(--violet));color:white;padding:10px 16px;font-size:0.78rem;font-weight:700;display:flex;align-items:center;gap:8px; }
.container-row-body { padding:1.1rem 1.25rem; }

.detail-grid { display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:1rem; }
.detail-item-lbl { font-size:0.68rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:var(--txt-3);margin-bottom:3px; }
.detail-item-val { font-size:0.9rem;font-weight:600;color:var(--txt-1); }
...
@media print {
    .sidebar,.top-nav,.page-header .btn-m,.no-print { display:none !important; }
...
<div class="summary-grid">
    ...
    <div class="summary-tile">
        ...
        <div class="summary-tile-lbl">Tare Weight (MT)</div>
    </div>
</div>
...
<div class="data-card">
    <div class="data-card-head">
        ...
        <div class="d-flex gap-2 no-print">
            <button onclick="setView('table')" id="btnTable" class="btn-m btn-ghost btn-sm">
            ...
            <table class="tbl">
                ...
                <tbody>
                    <?php foreach ($items as $i => $item):
                        $tare = (float)$item['gross_weight'] - (float)$item['net_weight'];
                    ?>
                    <tr>
                        ...
                        <td style="font-weight:600;white-space:nowrap;"><?php echo fmtMT($item['gross_weight']); ?></td>
                        <td style="font-weight:600;white-space:nowrap;"><?php echo fmtMT($item['net_weight']); ?></td>
                        <td style="font-weight:600;white-space:nowrap;color:var(--txt-2);"><?php echo fmtMT($tare); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr style="background:#f8fafc;border-top:2px solid var(--border);">
                        <td colspan="3" style="padding:12px 15px;font-weight:700;color:var(--txt-1);font-size:0.85rem;">TOTALS</td>
                ...
                <div class="container-row-body">
                    <div class="detail-grid">
                        ...
                        <div>
                            ...
                            <div class="detail-item-val" style="color:var(--rose);"><?php echo fmtMT($item['net_weight']); ?></div>
                        </div>
                        <div>
                            <div class="detail-item-lbl">Tare Weight</div>
                            <div class="detail-item-val" style="color:var(--txt-2);"><?php echo fmtMT($tare); ?></div>
                        </div>
                    </div>
                </div>
            </div>
