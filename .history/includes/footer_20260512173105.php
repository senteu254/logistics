    ...
    div.innerHTML = `
        ...
        <div class="row g-3">
            ...
            <div class="col-xl-4 col-md-6 col-12">
                <label class="f-label">Container No.<span class="req">*</span></label>
                <input type="text" name="container_number[]" class="f-input"
            ...
            <div class="col-xl-4 col-md-6 col-12">
                ...
                <input type="text" name="agent_seal_number[]" class="f-input"
                       placeholder="e.g. AS-00123">
            </div>

            <div class="col-xl-4 col-md-6 col-12">
                <label class="f-label">No. of Bags<span class="req">*</span></label>
                <input type="number" name="number_of_bags[]" class="f-input"
            ...
            <div class="col-xl-4 col-md-6 col-12">
                ...
                <input type="text" name="sgs_seal_number[]" class="f-input"
                       placeholder="e.g. SGS-00456">
            </div>

            <div class="col-xl-4 col-md-6 col-12">
                <label class="f-label">Gross Weight (kg)<span class="req">*</span></label>
                <input type="number" name="gross_weight[]" class="f-input"
                       placeholder="e.g. 25000.00" step="0.01" min="0" required
                       oninput="calcTotals()">
            </div>

            <div class="col-xl-4 col-md-6 col-12">
                <label class="f-label">Net Weight (kg)<span class="req">*</span></label>
                <input type="number" name="net_weight[]" class="f-input"
                       placeholder="e.g. 24500.00" step="0.01" min="0" required
                       oninput="calcTotals()">
            </div>
...
function removeRow(id) {
    ...
    setTimeout(() => { row.remove(); calcTotals(); reNumber(); }, 240);
}

function reNumber() {
    document.querySelectorAll('.item-row').forEach((r, i) => {
        const badge = r.querySelector('.row-num-badge');
        const label = r.querySelector('.row-num');
        if (badge) badge.textContent = i + 1;
        if (label) {
            const nodes = label.childNodes;
            nodes[nodes.length - 1].textContent = ` Container Row #${i + 1}`;
        }
    });
}
...
function calcTotals() {
    ...
    setEl('tGross', fmt(gross) + ' kg');
    setEl('tNet',   fmt(net)   + ' kg');
}

/* Init */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('rowsWrap')) addItemRow();
});
</script>
</body>
</html>